...
class TestColdStartDeferral:
    ...
    The pre-fixup state (PR 3 first commit) called load_secrets at
    ...
    These are source-text checks; runtime behavior is exercised by
    the docker smoke (not in CI).
    """
    ...
    def test_inference_handler_does_not_call_load_secrets_at_module_top(self):
        ...
        # The text-level check: load_secrets() call must appear AFTER def
        # _ensure_init( declaration, never at module-top.
    ...
    def test_inference_handler_calls_ensure_init_first_in_handler(self):
        text = (REPO_ROOT / "inference" / "handler.py").read_text()
        # Find the handler() body and confirm _ensure_init() is called
        # before any non-trivial work (preflight, daily_predict, etc.).
        handler_idx = text.find("def handler(")
        assert handler_idx != -1
        # Find the next call to _ensure_init() and the next call to
        # PredictorPreflight; ensure_init must come first.
