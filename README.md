...
**Current production model**: LightGBM GBM with sector-neutral labels.
...
1. A confidence-gated score modifier on the technical score
2. An ENTER-veto gate: predicted DOWN with sufficient confidence blocks new position entries
...
All hyperparameters are in `config/predictor.yaml` (gitignored — copy from `config/predictor.sample.yaml`).
The backtester auto-tunes the veto confidence threshold via `config/predictor_params.json` in S3.

Notable configuration dimensions:
...
- **Production gates**: Minimum IC and hit rate required for deployment
- **GBM hyperparameters**: Tree complexity, ensemble size, regularization
- **MLP architecture**: Hidden layer sizes, learning rate, early stopping
...
1. **Data**: Multi-year OHLCV parquets in `data/cache/` (one per ticker, plus SPY,
   VIX, sector ETFs, and `sector_map.json` generated by `bootstrap_fetcher.py`).
2. **Features**: `compute_features()` produces rolling technical + macro
   indicators per row. Warmup period required for long-window features.
3. **Labels (sector-neutral)**: `compute_labels()` computes the forward
   return relative to the ticker's **sector ETF** (not SPY):
   `(close[T+N]/close[T] - 1) - (sector_etf[T+N]/sector_etf[T] - 1)`.
...
5. **Split**: Time-based split (configurable ratios). Test set = most recent dates.
...
8. **Production gate**: Minimum IC required before uploading to S3 (threshold configurable).
